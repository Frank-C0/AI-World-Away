      ...
      <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-6 z-[500]">
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
          <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-5 flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold">📊 Estadísticas del Dataset</h2>
              <p className="text-blue-100 text-sm mt-1">Análisis completo de los datos</p>
            </div>
            {onClose && (
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <SummaryCard label="Total de Filas" value={stats.shape[0].toLocaleString()} icon="📋" gradient="from-cyan-600/60 to-cyan-400/40" />
        <SummaryCard label="Total de Columnas" value={stats.shape[1].toString()} icon="📊" gradient="from-purple-600/60 to-purple-400/40" />
        <SummaryCard label="Valores Nulos" value={stats.totalNulls.toLocaleString()} icon="⚠️" gradient="from-amber-600/60 to-amber-400/40" />
        <SummaryCard label="Completitud" value={((1 - stats.totalNulls / (stats.shape[0]*stats.shape[1]))*100).toFixed(1)+"%"} icon="✅" gradient="from-emerald-600/60 to-emerald-400/40" />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <ColumnPanel title={`🔢 Numéricas (${numeric.length})`} cols={numeric} panel={panel} sectionHead={sectionHead} dark={dark} type="numeric" />
        <ColumnPanel title={`🏷️ Categóricas (${categorical.length})`} cols={categorical} panel={panel} sectionHead={sectionHead} dark={dark} type="categorical" />
        <ColumnPanel title={`📝 Texto (${textCols.length})`} cols={textCols} panel={panel} sectionHead={sectionHead} dark={dark} type="text" />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className={tableHead}>
              <tr>
                {['Columna','Tipo','Categoría','Valores Únicos','Valores Nulos','Rango/Muestra'].map(h => (
                ...
                <tr key={col.name} className={dark ? 'hover:bg-cyan-400/5' : i % 2 ? 'bg-gray-50' : 'bg-white'}>
                  ...
                  <td className="px-5 py-2">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-semibold ${
                      ...
                      {col.isNumeric ? 'Numérica' : col.isCategorical ? 'Categórica' : 'Texto'}
                    </span>
                  </td>
...
const ColumnPanel: React.FC<{title: string; cols: DataStats['columns']; panel: string; sectionHead: string; dark: boolean; type: 'numeric'|'categorical'|'text';}> = ({title, cols, panel, sectionHead, dark, type}) => (
  <div className={`${panel} p-5`}>    
    ...
    <div className="space-y-3 max-h-60 overflow-y-auto custom-scrollbar pr-1 text-sm">
      {cols.length ? cols.map(col => (
        <div key={col.name} className={`p-3 rounded-md border ${dark ? 'border-cyan-400/20 bg-cyan-400/5 text-cyan-100' : type==='numeric'? 'bg-blue-50 border-blue-200 text-blue-800' : type==='categorical' ? 'bg-green-50 border-green-200 text-green-800' : 'bg-yellow-50 border-yellow-200 text-yellow-800'}`}>
          ...
          <div className={`${dark ? 'text-cyan-200/80' : ''} text-xs mt-1 space-y-0.5`}>            
            <div>Tipo: {col.dtype}</div>
            {col.isNumeric && col.min !== undefined && col.max !== undefined && (
              <div>Rango: {col.min.toLocaleString()} - {col.max.toLocaleString()}</div>
            )}
            {col.isCategorical && <div>Valores únicos: {col.uniqueValues.length}</div>}
            <div>Nulos: {col.nullCount}</div>
      ...
      )) : <p className="italic text-xs opacity-70">No hay columnas</p>}
    </div>
  </div>
);
